...
FOR SELECT 
USING (auth.email() = 'smart@example.com' OR (auth.jwt()->>'user_metadata')::json->>'display_name' = 'Smart');

CREATE POLICY "Smart user can insert versions" 
ON public.app_versions 
FOR INSERT 
WITH CHECK (auth.email() = 'smart@example.com' OR (auth.jwt()->>'user_metadata')::json->>'display_name' = 'Smart');
...
ON storage.objects 
FOR INSERT 
WITH CHECK (bucket_id = 'apk-files' AND (auth.email() = 'smart@example.com' OR (auth.jwt()->>'user_metadata')::json->>'display_name' = 'Smart'));
